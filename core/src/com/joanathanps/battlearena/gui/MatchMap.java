...
public class MatchMap implements Disposable {
    ...
    public void update(float delta, Vector2 playerPosition) {
        ...
        safezone.setPosition(
                match.getSafeZoneController().getExpandedSafezonePosition(safezone).x,
                match.getSafeZoneController().getExpandedSafezonePosition(safezone).y
        );
        safezone.setSize(
                match.getSafeZoneController().getExpandedSafezoneSize(),
                match.getSafeZoneController().getExpandedSafezoneSize()
        );

        Texture texture;
        if (match.getSafeZoneController().isInZoneEvent()) {
