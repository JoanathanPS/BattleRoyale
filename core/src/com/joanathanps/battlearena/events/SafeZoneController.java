...
import com.joanathanps.battlearena.entities.Player;
...
import static com.joanathanps.battlearena.scheme.MathUtils.distance;
import static com.joanathanps.battlearena.scheme.MathUtils.randomRange;
...
public class SafeZoneController {
    ...
    public SafeZoneController(Match match) {
        this.match = match;
    ...
    private void setupHud() {
        zoneLabel = match.getI18n().getBundle().get("nextZone");
        match.getHud().updateEventLabel(zoneLabel, isInZoneEvent());
    ...
    public void update(float delta) {
        if (event < 8) {
            ...
            if (secondsToNextEvent == -1) {
                event++;
                if (event == 2) {
                    zoneLabel = match.getI18n().getBundle().get("firstZone");
                } else if (event == 4) {
                    zoneLabel = match.getI18n().getBundle().get("secondZone");
                } else if (event == 6) {
                    zoneLabel = match.getI18n().getBundle().get("thirdZone");
                } else if (event == 8) {
                    zoneLabel = match.getI18n().getBundle().get("finalZone");
                } else {
                    zoneLabel = match.getI18n().getBundle().get("nextZone");
                }
                match.getHud().updateEventLabel(zoneLabel, isInZoneEvent());
